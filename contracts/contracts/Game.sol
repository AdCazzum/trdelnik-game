...
contract TrdelnikGame is ReentrancyGuard {
    /* ────────────────────────  Types  ───────────────────────── */
    ...
    RandomNumberV2Interface internal _generator;
    uint16 public _secretNumber;
    uint256 public _currentTimestamp;
    ...
    uint16 private constant PROB_EASY = 9000; // 96.00 %
    uint16 private constant PROB_MEDIUM = 8500; // 88.00 %
    uint16 private constant PROB_HARD = 7000; // 80.00 %
    uint16 private constant PROB_HARDCORE = 5500; // 60.00 %
    ...
    function _requestStep(uint256 gameId) internal {
        ...
        _setNewSecretNumber(0, 10000);
        _resolveStep(gameId, _secretNumber);
    }

    function _resolveStep(uint256 gameId, uint16 secretNumber) internal {
        Game storage g = games[gameId];
        uint16 threshold = _probability(g.difficulty);
        
        bool success = _isWin(secretNumber, threshold);
    ...
    function _setNewSecretNumber(uint256 min, uint256 max) private {
        require(max >= min, "invalid-range");

        (uint256 randomNumber, bool isSecure, uint256 timestamp) = _generator.getRandomNumber();
        require(isSecure, "unable to generate a secure number");
        require(timestamp != _currentTimestamp, "no new random number have been generated");

        uint256 range = max - min + 1;
        uint256 randomInRange = min + (randomNumber % range);

        _secretNumber = uint16(randomInRange);
        _currentTimestamp = timestamp;
    }

    function _isWin(uint256 random, uint256 threshold) internal pure returns (bool) {
        // Map random number in range [0, 9999]
